...
<div class="fixed inset-0 z-[130] bg-black/85 flex items-center justify-center p-8"
  ...
  <div data-focus-trap
    ...
    <div class="flex items-center gap-4">
      ...
      <div>
        <h2 class="text-3xl font-bold text-white">{$t.syncPlay}</h2>
        <p class="text-gray-400 text-sm mt-1">{$t.syncPlayIntro}</p>
      </div>
    </div>
    ...
    {#if group}
      ...
      <div class="bg-gray-800/70 border border-gray-700 rounded-xl p-5 flex flex-col gap-3">
        <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider">{$t.syncGroupActive}</span>
        <span class="text-xl font-bold text-white">{group.GroupName}</span>
        <div class="flex flex-col gap-1">
          <span class="text-xs font-semibold text-gray-500 uppercase tracking-wider">{$t.groupMembers}</span>
    ...
    {:else}
      ...
      <div class="flex flex-col gap-2">
        ...
        {#if groups.length > 0}
          ...
          {/each}
        {:else}
          <p class="text-gray-500 text-center py-6">{loading ? '…' : $t.noSyncGroups}</p>
        {/if}
      </div>
    {/if}
